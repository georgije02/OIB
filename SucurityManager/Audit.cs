...
namespace SecurityManager
{
    public class Audit : IDisposable
    {
        ...
        public static void NoCertificateCDB()
        {
            if (customLog != null)
            {
                string ErrorCertifikate = AuditEvents.NoCertificateCDB;
                string message = String.Format(ErrorCertifikate);
                customLog.WriteEntry(message, EventLogEntryType.Error);
            }
            else
            {
                throw new ArgumentException(string.Format("Error while trying to write event (eventid = {0}) to event log.",
                    (int)AuditEventTypes.NoCertificateCDB));
            }
        }

        public static void NoCertificateLDB()
        {
            if (customLog != null)
            {
                string ErrorCertifikate = AuditEvents.NoCertificateLDB;
                string message = String.Format(ErrorCertifikate);
                customLog.WriteEntry(message, EventLogEntryType.Error);
            }
            else
            {
                throw new ArgumentException(string.Format("Error while trying to write event (eventid = {0}) to event log.",
                    (int)AuditEventTypes.NoCertificateLDB));
            }
        }
